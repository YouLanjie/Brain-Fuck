...
{				/*{{{ */
	...
	cmenu_set_title(data, "首页");
	cmenu_add_text(data, 0, "1.执行代码",
	...
	cmenu_add_text(data, 0, "2.执行上一次",
		       "这会读取上一次执行的程序留下的临时文件并自动执行，"
		       "倘若没有临时文件则会报错",
		       NULL, NULL, NULL, 0, 0, 0);
	cmenu_add_text(data, 0, "3.程序帮助", "查看程序的帮助条例", NULL, NULL, NULL, 0, 0, 0);
	cmenu_add_text(data, 0, "4.程序设置", "关于设置", NULL, NULL, NULL, 0, 0, 0);
	cmenu_add_text(data, 0, "5.删除临时文件", "移除程序的临时文件", NULL, NULL, NULL, 0, 0, 0);
	cmenu_add_text(data, 0, "0.退出", "退出程序", NULL, NULL, NULL, 0, 0, 0);
